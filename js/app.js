...
async function openFinalizarModal(ag) {
  ...
  // Busca TODOS os pacotes ativos do cliente (não só os que cobrem o serviço exato)
  ...
  if (pacotes && pacotes.length > 0) {
    pacotesHTML = pacotes.map(p => {
      // Acha um item disponível
      const itemExato = p.itens.find(i => i.servico_id === ag.servico_id && i.quantidade_usada < i.quantidade_total);
      const itemQualquer = p.itens.find(i => i.quantidade_usada < i.quantidade_total);
      const item = itemExato || itemQualquer;

      if (!item) return ''; // pacote sem itens disponíveis

      const restantes = item.quantidade_total - item.quantidade_usada;
      const isExato = !!itemExato;
      const itemServicoNome = item.servico?.nome || "Serviço";

      return `
        <button class="${isExato ? 'card-gold' : 'card'}" style="text-align:left;cursor:pointer;font-family:inherit;color:var(--text);${isExato ? 'border:1px solid rgba(212,168,87,0.5)' : 'border-color:var(--line)'}" onclick='${isExato ? '' : `if (!confirm("⚠️ Esse pacote é de '${itemServicoNome}', e você está finalizando '${servicoNome}'. Descontar mesmo assim?")) return;`}confirmarFinalizar(${escapeJsonForAttr({ agendamento_id: ag.id, cliente_id: ag.cliente_id, servico_id: ag.servico_id, pago_via_pacote: true, pacote_id: p.id, pacote_item_id: item.id })})'>
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px">
            <div style="font-weight:600;font-size:14px;display:flex;align-items:center;gap:8px">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="${isExato ? 'var(--gold)' : 'var(--text-soft)'}" stroke-width="2"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg>
              ${escapeHtml(p.nome)}
            </div>
            ${isExato ? '<span class="pill pill-gold" style="font-size:10px">Recomendado</span>' : '<span class="pill pill-dim" style="font-size:10px">Serviço diferente</span>'}
          </div>
          <div style="font-size:12px;color:var(--text-soft);line-height:1.5">
            ${restantes} ${itemServicoNome.toLowerCase()}${restantes !== 1 ? 's' : ''} restante${restantes !== 1 ? 's' : ''} · sem cobrança hoje
            ${!isExato ? `<br><span style="color:var(--text-dim);font-size:11px">⚠️ pacote de "${itemServicoNome}", agendamento de "${servicoNome}"</span>` : ''}
          </div>
        </button>
      `;
    }).filter(Boolean).join("");
  ...
  document.getElementById("finalizarBody").innerHTML = `
    ${pacotesHTML ? `
      <div style="font-size:11px;letter-spacing:1.5px;text-transform:uppercase;color:var(--gold);font-weight:600;margin-bottom:10px">Usar pacote</div>
