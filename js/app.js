...
async function renderCobranca() {
  ...
  container.innerHTML = `
    ...
    <!-- Suporte -->
    <div class="card" style="text-align:center;padding:18px;background:var(--bg-soft);border-color:var(--line-soft)">
      <div style="font-size:13px;color:var(--text-soft);margin-bottom:8px">Precisa de ajuda com sua cobrança?</div>
      <a href="https://wa.me/55${(state.profile?.phone || '').replace(/\\D/g,'')}?text=Olá! Preciso de ajuda com minha cobrança no Cortify." target="_blank" class="btn btn-ghost btn-sm">
        💬 Falar com suporte
      </a>
    </div>
  `;
