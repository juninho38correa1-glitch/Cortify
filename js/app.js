...
// ABA COBRANÇA
// ============================================
async function renderCobranca() {
  const container = document.getElementById("contaContent");
  container.innerHTML = `<div style="padding:30px;text-align:center;color:var(--text-soft)">Carregando...</div>`;

  // 1. Checa status de acesso (sabe se é membro pago pela barbearia)
  const { data: access } = await sb.rpc('my_access_status');

  // 2. Se é BARBER/MANAGER/RECEPTIONIST de barbearia → mostra tela "pago pela barbearia"
  if (access?.reason === 'PAID_BY_BARBERSHOP') {
    return renderCobrancaMember(access);
  }

  // 3. Senão, segue fluxo normal (OWNER/AUTONOMO)
  const { data: info, error } = await sb.rpc('my_subscription_info');

  if (error || !info) {
    container.innerHTML = `
      <div class="card" style="text-align:center;padding:40px;color:var(--text-soft)">
        Erro ao carregar informações de cobrança.
      </div>
    `;
    return;
  }
  ...
  if (!info.has_subscription) {
    container.innerHTML = `
      <div class="card" style="text-align:center;padding:40px">
        ...
        <h3 style="font-family:'Playfair Display';margin-bottom:8px">Sem assinatura ativa</h3>
        <p style="color:var(--text-soft);font-size:13px">Sua conta não tem uma assinatura registrada.</p>
      </div>
    `;
    return;
  }
  ...
  // Formata status
  const statusInfo = {
    'TRIAL': { label: 'Em trial', color: '#6fcf97', icon: '🎁' },
    'ACTIVE': { label: 'Ativa', color: '#6fcf97', icon: '✅' },
    'PAST_DUE': { label: 'Pagamento atrasado', color: '#ffb73e', icon: '⚠️' },
    'CANCELLED': { label: 'Cancelada', color: '#e07474', icon: '❌' },
    'EXPIRED': { label: 'Expirada', color: '#e07474', icon: '⏰' },
    'BLOCKED': { label: 'Bloqueada', color: '#e07474', icon: '🔒' },
  };
  const status = statusInfo[info.status] || { label: info.status, color: 'var(--text-soft)', icon: 'ℹ️' };

  // Formata datas
  const fmtDate = (dt) => dt ? new Date(dt).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric' }) : '—';
  const trialEndsAt = info.trial_ends_at ? new Date(info.trial_ends_at) : null;
  const isInTrial = info.status === 'TRIAL' && trialEndsAt && trialEndsAt > new Date();
  const daysLeft = isInTrial ? Math.ceil((trialEndsAt - new Date()) / 86400000) : 0;

  // Determina próxima cobrança
  const nextChargeDate = info.current_period_end || info.trial_ends_at;

  // Detecta se é barbearia OWNER (pode ter breakdown detalhado)
  const isBarbearia = info.plan_kind === 'BARBEARIA';
  ...
  container.innerHTML = `
    ...
    <div class="card" style="margin-bottom:20px">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:14px;margin-bottom:18px;flex-wrap:wrap">
        <div>
          <div style="font-size:11px;color:var(--text-dim);letter-spacing:1.5px;text-transform:uppercase;font-weight:600;margin-bottom:6px">Seu plano</div>
          <div style="font-family:'Playfair Display', serif;font-size:26px;font-weight:700">${escapeHtml(info.plan_name)}</div>
          <div style="font-size:12px;color:var(--text-soft);margin-top:4px">${info.billing_cycle === 'YEARLY' ? 'Cobrança anual' : 'Cobrança mensal'}</div>
        </div>
        <div style="text-align:right">
          <div style="display:inline-block;padding:6px 14px;border-radius:20px;font-size:12px;font-weight:600;background:${status.color}22;color:${status.color};border:1px solid ${status.color}55">
      ...
      ${isInTrial ? `
        <div style="background:rgba(111,207,151,0.08);border:1px solid rgba(111,207,151,0.25);border-radius:10px;padding:14px;margin-bottom:18px">
          <div style="display:flex;align-items:center;gap:10px">
            <div style="font-size:24px">🎁</div>
            <div>
              <div style="font-weight:600;color:var(--green)">Você está em trial</div>
              <div style="font-size:12px;color:var(--text-soft);margin-top:2px">
                ${daysLeft} dia${daysLeft !== 1 ? 's' : ''} restante${daysLeft !== 1 ? 's' : ''} 
                · termina em ${fmtDate(trialEndsAt)}
              </div>
            </div>
          </div>
        </div>
      ...
      <div style="display:grid;gap:14px;grid-template-columns:1fr 1fr;margin-bottom:14px">
        <div style="background:var(--bg-soft);padding:14px;border-radius:10px">
          <div style="font-size:10px;color:var(--text-dim);letter-spacing:1.5px;text-transform:uppercase;font-weight:600;margin-bottom:6px">Valor mensal</div>
          <div style="font-family:'Playfair Display';font-size:28px;font-weight:700;color:var(--gold)">${bf.formatBRL(info.amount)}</div>
          ${isBarbearia ? `<div style="font-size:11px;color:var(--text-soft);margin-top:4px">${escapeHtml(info.breakdown)}</div>` : ''}
        </div>
        <div style="background:var(--bg-soft);padding:14px;border-radius:10px">
          <div style="font-size:10px;color:var(--text-dim);letter-spacing:1.5px;text-transform:uppercase;font-weight:600;margin-bottom:6px">Próxima cobrança</div>
          <div style="font-family:'Playfair Display';font-size:22px;font-weight:700">${fmtDate(nextChargeDate)}</div>
          ${info.status === 'PAST_DUE' ? `<div style="font-size:11px;color:var(--red);margin-top:4px">⚠️ Pagamento atrasado</div>` : ''}
        </div>
      </div>

      ${isBarbearia ? `
        <div style="background:rgba(212,168,87,0.05);border:1px solid rgba(212,168,87,0.2);border-radius:10px;padding:14px;margin-bottom:14px">
          <div style="font-size:11px;color:var(--gold);letter-spacing:1px;text-transform:uppercase;font-weight:600;margin-bottom:8px">💰 Detalhes do preço</div>
          <div style="font-size:13px;line-height:1.8">
            <div style="display:flex;justify-content:space-between"><span style="color:var(--text-soft)">Plano base (1 barbeiro):</span> <strong>${bf.formatBRL(info.base_price)}</strong></div>
            ${info.addon_seats > 0 ? `
              <div style="display:flex;justify-content:space-between"><span style="color:var(--text-soft)">+ ${info.addon_seats} barbeiro(s) adicional:</span> <strong>${bf.formatBRL(info.addon_price_per_seat * info.addon_seats)}</strong></div>
            ` : ''}
            <div style="display:flex;justify-content:space-between;border-top:1px solid var(--line);padding-top:8px;margin-top:8px"><strong>Total:</strong> <strong style="color:var(--gold)">${bf.formatBRL(info.amount)}/mês</strong></div>
          </div>
          <div style="font-size:11px;color:var(--text-dim);margin-top:10px;font-style:italic">
            ℹ️ Cada barbeiro adicional adiciona ${bf.formatBRL(info.addon_price_per_seat)}/mês automaticamente.
          </div>
        </div>
      ` : ''}
      ...
      ${info.features ? `
        <div>
          <div style="font-size:11px;color:var(--text-dim);letter-spacing:1.5px;text-transform:uppercase;font-weight:600;margin-bottom:10px">O que está incluso</div>
          <div style="display:grid;grid-template-columns:repeat(auto-fill, minmax(180px, 1fr));gap:6px">
            ${info.features.can_use_public_page ? '<div style="font-size:12px;color:var(--text-soft)">✅ Página pública</div>' : ''}
            ${info.features.can_use_packages ? '<div style="font-size:12px;color:var(--text-soft)">✅ Pacotes de serviço</div>' : ''}
            ${info.features.can_have_logo ? '<div style="font-size:12px;color:var(--text-soft)">✅ Logo customizada</div>' : ''}
            ${info.features.can_manage_team ? '<div style="font-size:12px;color:var(--text-soft)">✅ Gestão de equipe</div>' : ''}
            ${info.features.can_view_team_dashboard ? '<div style="font-size:12px;color:var(--text-soft)">✅ Dashboard da equipe</div>' : ''}
            ${info.features.can_export_pdf ? '<div style="font-size:12px;color:var(--text-soft)">✅ Export PDF</div>' : ''}
            ${info.features.can_use_loyalty_system ? '<div style="font-size:12px;color:var(--text-soft)">✅ Programa fidelidade</div>' : ''}
            ${info.features.can_send_email_marketing ? '<div style="font-size:12px;color:var(--text-soft)">✅ Email marketing</div>' : ''}
            ${info.features.can_use_whatsapp_api ? '<div style="font-size:12px;color:var(--text-soft)">✅ WhatsApp API</div>' : ''}
          </div>
        </div>
      ` : ''}
    </div>
    ...
    ${info.has_asaas ? `
      <div class="card" style="margin-bottom:20px">
        <div style="margin-bottom:14px">
          <div style="font-family:'Playfair Display';font-size:16px;font-weight:700;margin-bottom:4px">💳 Forma de pagamento</div>
          <div style="font-size:12px;color:var(--text-soft)">Sua cobrança é processada pelo Asaas. Atualize seu cartão ou forma de pagamento diretamente lá.</div>
        </div>
        <a href="https://www.asaas.com/customer/area/" target="_blank" class="btn btn-ghost" style="font-size:13px">
          🔗 Abrir painel Asaas
        </a>
        <p style="font-size:11px;color:var(--text-dim);margin-top:10px">Login: ${escapeHtml(state.profile?.email || '')}</p>
      </div>
    ` : `
      <div class="card" style="margin-bottom:20px;background:rgba(255,183,62,0.05);border-color:rgba(255,183,62,0.25)">
        <div style="display:flex;align-items:center;gap:10px">
          <div style="font-size:24px">⚠️</div>
          <div>
            <div style="font-weight:600;color:#ffb73e">Cobrança não sincronizada</div>
            <div style="font-size:12px;color:var(--text-soft);margin-top:2px">
              Sua assinatura está registrada no sistema, mas ainda não foi configurada no Asaas. Entre em contato.
            </div>
          </div>
        </div>
      </div>
...
// Renderiza tela "pago pela barbearia" pra BARBER/MANAGER/RECEPTIONIST
async function renderCobrancaMember(access) {
  const container = document.getElementById("contaContent");
  
  // Busca info do OWNER pra mostrar nome
  let ownerName = '';
  if (access.paid_by) {
    const { data: owner } = await sb.from('profiles').select('name').eq('id', access.paid_by).maybeSingle();
  ...
  container.innerHTML = `
    <!-- Card principal: você é membro -->
    <div class="card" style="margin-bottom:20px">
      <div style="text-align:center;padding:24px 0 14px">
        <div style="font-size:56px;margin-bottom:10px">🏪</div>
        <div style="font-family:'Playfair Display', serif;font-size:24px;font-weight:700;margin-bottom:6px">
          ${escapeHtml(access.barbershop_name)}
        </div>
        <div style="display:inline-block;padding:6px 14px;border-radius:20px;font-size:12px;font-weight:600;background:rgba(111,207,151,0.15);color:var(--green);border:1px solid rgba(111,207,151,0.35)">
          ✅ Acesso liberado
        </div>
      </div>

      <div style="background:rgba(111,207,151,0.06);border:1px solid rgba(111,207,151,0.2);border-radius:10px;padding:16px;margin:18px 0">
        <div style="display:flex;align-items:flex-start;gap:12px">
          <div style="font-size:24px">💰</div>
          <div style="flex:1">
            <div style="font-weight:600;color:var(--green);margin-bottom:4px">Sem cobrança pra você</div>
            <div style="font-size:13px;color:var(--text-soft);line-height:1.6">
              Você é <strong>${roleLabelLower}</strong> da barbearia <strong>${escapeHtml(access.barbershop_name)}</strong>. 
              ${ownerName ? `O dono ${escapeHtml(ownerName.split(' ')[0])}` : 'O dono'} paga a mensalidade do sistema por toda a equipe.
            </div>
          </div>
        </div>
      </div>
      ...
      <div style="display:grid;gap:10px;font-size:13px">
        <div style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--line-soft)">
          <span style="color:var(--text-soft)">Sua função:</span>
          <strong>${roleLabel(access.role)}</strong>
        </div>
        <div style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--line-soft)">
          <span style="color:var(--text-soft)">Barbearia:</span>
          <strong>${escapeHtml(access.barbershop_name)}</strong>
        </div>
        <div style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--line-soft)">
          <span style="color:var(--text-soft)">Status da barbearia:</span>
          <span style="color:var(--green);font-weight:600">${access.owner_status === 'ACTIVE' ? '✅ Em dia' : (access.owner_status === 'TRIAL' ? '🎁 Trial' : access.owner_status)}</span>
        </div>
        <div style="display:flex;justify-content:space-between;padding:10px 0">
          <span style="color:var(--text-soft)">Seu valor pago:</span>
    ...
    <!-- Info adicional -->
    <div class="card" style="margin-bottom:20px;background:var(--bg-soft);border-color:var(--line-soft)">
      <div style="display:flex;align-items:flex-start;gap:12px">
        <div style="font-size:20px">ℹ️</div>
        <div style="font-size:12px;color:var(--text-soft);line-height:1.7">
          <strong style="color:var(--text)">Como funciona:</strong> Quando você é membro de uma barbearia no Cortify, o dono assume o custo da assinatura por toda a equipe. Você tem acesso completo enquanto for membro ativo.<br><br>
          <strong style="color:var(--text)">Saiu da barbearia?</strong> Se você deixar de fazer parte da equipe, sua conta volta a ser autônoma e você pode escolher um plano próprio.
        </div>
      </div>
    </div>
  `;
...
async function renderPaginaPublica() {
  const container = document.getElementById("contaContent");
  container.innerHTML = `<div style="padding:30px;text-align:center;color:var(--text-soft)">Carregando...</div>`;

  // Recarrega o perfil pra pegar campos públicos
  const { data: profile } = await sb
    .from('profiles')
    .select('slug, public_page_active, public_bio, work_schedule')
    .eq('id', state.user.id)
    .maybeSingle();

  const slug = profile?.slug || '';
  const ativa = profile?.public_page_active || false;
  const bio = profile?.public_bio || '';
  const workSchedule = profile?.work_schedule || {
    ...
    "6": {"active": true, "start": "09:00", "end": "16:00"}
  };
  ...
  const publicUrl = slug ? `${window.location.origin}/book.html?b=${slug}` : '';
  ...
  container.innerHTML = `
    <div class="card-gradient" style="margin-bottom:20px">
      <div class="block-h">
        ...
        <span class="pill ${ativa ? 'pill-green' : 'pill-dim'}">${ativa ? 'ATIVA' : 'INATIVA'}</span>
      </div>
      <p style="font-size:13px;color:var(--text-soft);margin-bottom:14px;line-height:1.5">
        Crie um link pra seus clientes marcarem horário sozinhos, sem precisar te chamar no Whats. Funciona 24/7.
      </p>
      <button class="btn ${ativa ? 'btn-danger' : 'btn-primary'}" onclick="togglePaginaPublica(${!ativa})">
        ${ativa ? 'Desativar página' : 'Ativar página'}
      </button>
    </div>

    <div class="card" style="margin-bottom:20px">
      <div class="block-h"><h3>Configuração</h3></div>
      <form id="publicForm" onsubmit="salvarConfigPublica(event)">
        <div class="field">
          <label>Seu nome único na URL <span style="color:var(--red)">*</span></label>
          <div style="display:flex;align-items:center;gap:0;background:var(--bg-soft);border:1.5px solid var(--line);border-radius:10px;overflow:hidden">
            <span style="padding:11px 12px;font-size:12px;color:var(--text-dim);font-family:'JetBrains Mono', monospace">${window.location.origin}/?b=</span>
            <input class="input" type="text" name="slug" value="${escapeHtml(slug)}" placeholder="marcos" required pattern="[a-z0-9-]+" style="border:none;border-radius:0;padding-left:0;background:transparent">
          </div>
          <p class="field-hint">Só letras minúsculas, números e hífens. Ex: marcos, dom-marcos, mc-cortes</p>
        </div>
        <div class="field">
          <label>Descrição (opcional)</label>
          <textarea class="textarea" name="bio" rows="3" placeholder="Especialista em cortes degradê e barba. Atendimento personalizado.">${escapeHtml(bio)}</textarea>
          <p class="field-hint">Aparece pro cliente na página de agendamento</p>
        </div>
    ...
    ${ativa && slug ? `
      <div class="card" style="margin-bottom:20px;background:rgba(212,168,87,0.04);border-color:rgba(212,168,87,0.25)">
        <div class="block-h"><h3 style="color:var(--gold)">Seu link público</h3></div>
        <div style="background:var(--bg-soft);border:1px solid var(--line);border-radius:10px;padding:12px;display:flex;align-items:center;gap:10px;margin-bottom:12px">
          <code style="flex:1;font-family:'JetBrains Mono', monospace;font-size:12px;color:var(--gold);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${publicUrl}</code>
          <button class="btn btn-ghost btn-sm" onclick="copiarLink('${publicUrl}')" style="flex-shrink:0">Copiar</button>
        </div>
        <div style="display:flex;gap:8px;flex-wrap:wrap">
          <a href="${publicUrl}" target="_blank" class="btn btn-ghost btn-sm">Abrir página</a>
          <a href="https://wa.me/?text=${encodeURIComponent(`Marca seu horário pelo meu link: ${publicUrl}`)}" target="_blank" class="btn btn-ghost btn-sm">Compartilhar</a>
        </div>
      </div>
    ` : ''}

    <div class="card" style="margin-bottom:20px">
      <div class="block-h"><h3>Horário de funcionamento</h3></div>
      <p style="font-size:12px;color:var(--text-dim);margin-bottom:14px">Define quais dias e horas você atende. Cliente só verá esses slots.</p>
      <div id="workScheduleEditor">
        ${[1, 2, 3, 4, 5, 6, 0].map(d => {
          const cfg = workSchedule[String(d)] || { active: false, start: '09:00', end: '18:00' };
          return `
            <div style="display:grid;grid-template-columns:90px 1fr;gap:10px;padding:10px 0;border-bottom:1px solid var(--line-soft);align-items:center">
              <div style="display:flex;align-items:center;gap:8px">
                <div class="toggle-switch ${cfg.active ? 'on' : ''}" onclick="toggleWorkDay(${d}, this)">
                  <div class="toggle-knob"></div>
                </div>
                <span style="font-size:13px;font-weight:500">${diasNomes[d].slice(0, 3)}</span>
              </div>
              <div style="display:flex;gap:6px;align-items:center;${cfg.active ? '' : 'opacity:0.4;pointer-events:none'}">
                <input type="time" class="input" value="${cfg.start}" onchange="updateWorkTime(${d}, 'start', this.value)" style="padding:8px;font-size:12px;width:auto">
                <span style="color:var(--text-dim);font-size:12px">até</span>
                <input type="time" class="input" value="${cfg.end}" onchange="updateWorkTime(${d}, 'end', this.value)" style="padding:8px;font-size:12px;width:auto">
              </div>
            </div>
          `;
        }).join('')}
    ...
    <div class="card" style="margin-bottom:20px">
      <div class="block-h">
        <h3>Folgas / bloqueios</h3>
        <button class="btn btn-ghost btn-sm" onclick="openBlockModal()">+ Adicionar</button>
      </div>
      <p style="font-size:12px;color:var(--text-dim);margin-bottom:14px">Bloqueie períodos específicos (feriados, viagens, etc).</p>
      ${!blocks || blocks.length === 0 ? `
        <p style="text-align:center;color:var(--text-dim);font-size:13px;padding:20px">Nenhum bloqueio cadastrado.</p>
      ` : blocks.map(b => `
        <div style="display:flex;justify-content:space-between;align-items:center;padding:10px;background:var(--bg-soft);border-radius:8px;margin-bottom:8px">
          <div>
            <div style="font-size:13px;font-weight:500">${formatDateBR(b.inicio)} ${new Date(b.inicio).toLocaleTimeString('pt-BR', {hour: '2-digit', minute: '2-digit'})} - ${formatDateBR(b.fim)} ${new Date(b.fim).toLocaleTimeString('pt-BR', {hour: '2-digit', minute: '2-digit'})}</div>
            ${b.motivo ? `<div style="font-size:11px;color:var(--text-dim);margin-top:2px">${escapeHtml(b.motivo)}</div>` : ''}
          </div>
          <button class="icon-btn" onclick="deleteBlock('${b.id}')" title="Remover" style="color:var(--red)">
  ...
  // Salva o workSchedule no state pra usar nos handlers
  state.currentWorkSchedule = workSchedule;
}

async function togglePaginaPublica(ativar) {
  // Se for ativar, precisa ter slug definido
  if (ativar) {
    const { data: p } = await sb.from('profiles').select('slug').eq('id', state.user.id).maybeSingle();
    if (!p?.slug) {
      bf.toast('Defina seu nome único na URL antes de ativar', 'error');
      return;
    }
  }
  ...
  const { error } = await sb
    .from('profiles')
    .update({ public_page_active: ativar })
    .eq('id', state.user.id);
  ...
  bf.toast(ativar ? 'Página pública ativada! 🎉' : 'Página pública desativada', 'success');
  renderPaginaPublica();
}

async function salvarConfigPublica(e) {
  e.preventDefault();
  const fd = new FormData(e.target);
  ...
  if (!/^[a-z0-9-]+$/.test(slug)) {
    bf.toast('Use só letras minúsculas, números e hífens no slug', 'error');
    return;
  }

  // Verifica se o slug já tá em uso por outro user
  ...
  if (existing) {
    bf.toast('Esse nome já está em uso. Escolha outro.', 'error');
    return;
  }

  const { error } = await sb
    .from('profiles')
    .update({ slug, public_bio: bio || null })
    .eq('id', state.user.id);
  ...
  bf.toast('Configurações salvas', 'success');
  renderPaginaPublica();
}

async function toggleWorkDay(dia, el) {
  const ativo = !el.classList.contains('on');
  el.classList.toggle('on');

  if (!state.currentWorkSchedule[String(dia)]) {
    state.currentWorkSchedule[String(dia)] = { active: ativo, start: '09:00', end: '18:00' };
  } else {
    state.currentWorkSchedule[String(dia)].active = ativo;
  }

  await saveWorkSchedule();

  // Atualiza UI sem re-render completo
  const row = el.closest('div').parentElement.nextElementSibling;
  row.style.opacity = ativo ? '1' : '0.4';
  row.style.pointerEvents = ativo ? 'auto' : 'none';
}

async function updateWorkTime(dia, campo, valor) {
  if (!state.currentWorkSchedule[String(dia)]) {
    state.currentWorkSchedule[String(dia)] = { active: true, start: '09:00', end: '18:00' };
  }
  state.currentWorkSchedule[String(dia)][campo] = valor;
  await saveWorkSchedule();
}
...
async function saveWorkSchedule() {
  const { error } = await sb
    .from('profiles')
    .update({ work_schedule: state.currentWorkSchedule })
    .eq('id', state.user.id);

  if (error) {
    bf.toast('Erro ao salvar horário: ' + error.message, 'error');
  }
}
...
function copiarLink(url) {
  navigator.clipboard.writeText(url).then(() => {
    ...
    bf.toast('Erro ao copiar', 'error');
  });
}

function openBlockModal() {
  const html = `
    <form onsubmit="criarBlock(event)" style="text-align:left">
      <div class="field">
        <label>De *</label>
        <input class="input" type="datetime-local" name="inicio" required>
      </div>
      <div class="field">
        <label>Até *</label>
        <input class="input" type="datetime-local" name="fim" required>
      </div>
      <div class="field">
        <label>Motivo (opcional)</label>
        <input class="input" type="text" name="motivo" placeholder="Feriado, viagem, etc">
      </div>
      <button type="submit" class="btn btn-primary btn-sm" style="margin-top:8px">Salvar bloqueio</button>
      <button type="button" class="btn btn-ghost btn-sm" onclick="closeBlockModal()" style="margin-top:8px;margin-left:8px">Cancelar</button>
    </form>
  `;

  const overlay = document.createElement('div');
  overlay.id = 'blockModalOverlay';
  ...
  overlay.innerHTML = `
    <div style="background:#131313;border:1px solid #232323;border-radius:14px;padding:24px;max-width:420px;width:100%">
      <h3 style="font-family:'Playfair Display';font-size:20px;font-weight:600;margin-bottom:14px">Bloquear horário</h3>
      ${html}
    </div>
  `;
  document.body.appendChild(overlay);
...
function closeBlockModal() {
  document.getElementById('blockModalOverlay')?.remove();
}

async function criarBlock(e) {
  e.preventDefault();
  const fd = new FormData(e.target);
  ...
  if (new Date(fim) <= new Date(inicio)) {
    bf.toast('A data fim deve ser depois do início', 'error');
    return;
  }
  ...
  bf.toast('Bloqueio criado', 'success');
  closeBlockModal();
  renderPaginaPublica();
}
...
async function deleteBlock(id) {
  ...
  bf.toast('Bloqueio removido', 'success');
  renderPaginaPublica();
}
...
async function confirmarVirarBarbearia(e) {
  ...
  // Pega preview de cobrança ANTES de confirmar
  let billingPreview = null;
  try {
    const { data: prev } = await sb.rpc('preview_barbershop_billing');
    if (prev && !prev.error) {
      billingPreview = prev;
    }
  } catch (e) { /* ok, não bloqueia */ }

  // Mensagem de confirmação com info de cobrança
  let migracaoLabel = '';
  ...
  let billingMsg = '';
  if (billingPreview?.new_plan) {
    const np = billingPreview.new_plan;
    billingMsg = `\n\n💰 NOVO PLANO:\n• ${np.starting_breakdown}\n• ${np.addon_explanation}\n• Sua cobrança atual será cancelada e uma nova será criada`;
  }

  if (!confirm(
    `Confirmar criação da barbearia "${name}"?\n\n` +
    `• ${migracaoLabel}\n` +
    `• Você vira o Dono\n` +
    `• Sua conta muda de Autônomo pra Barbearia${billingMsg}`
  )) {
    btn.disabled = false;
  ...
  btn.textContent = 'Criando barbearia...';

  // 1. Cria barbearia + migra subscription (banco)
  const { data, error } = await sb.rpc('create_barbershop_with_billing', {
    p_name: name,
    p_slug: slug,
  ...
  if (data && !data.success) {
    bf.toast('Erro: ' + (data.error || 'desconhecido'), 'error');
    btn.disabled = false;
    btn.textContent = 'Criar Barbearia';
    return;
  }

  // 2. Sincroniza Asaas (chama Edge Function)
  if (data.needs_asaas_sync && data.subscription_id) {
    btn.textContent = 'Atualizando cobrança...';
    try {
      const session = await sb.auth.getSession();
      const accessToken = session?.data?.session?.access_token;
      
      const syncRes = await fetch(
        `${window.SUPABASE_CONFIG?.url || ''}/functions/v1/asaas-sync-subscription`,
        {
          method: 'POST',
          headers: {
      ...
      if (syncRes.ok && syncData.ok) {
        console.log('✅ Asaas sincronizado:', syncData);
      } else {
        console.warn('⚠️ Asaas não sincronizou:', syncData);
        // Não falha o fluxo, só avisa
        bf.toast('⚠️ Barbearia criada, mas a cobrança não sincronizou. Entre em contato.', 'error');
      }
    } catch (e) {
      console.warn('Erro ao sincronizar Asaas:', e);
      // Não falha o fluxo
    }
  }

  // 3. SUCESSO!
  const migrated = data.migration_data || {};
  bf.toast(
  ...
  setTimeout(() => window.location.href = 'app.html#conta', 2000);
}
...
function setContaTab(tab) {
  ...
  document.getElementById("cfgTabCobranca")?.classList.toggle("active", tab === "cobranca");
  renderConta();
}
...
// Página pública
window.togglePaginaPublica = togglePaginaPublica;
window.salvarConfigPublica = salvarConfigPublica;
...
window.renderCobrancaMember = renderCobrancaMember;
