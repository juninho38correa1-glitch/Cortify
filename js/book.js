  ...
  container.innerHTML = state.servicos.map(s => `
    <div class="servico-item" data-id="${s.id}" onclick="selecionarServico('${s.id}')">
      <div class="servico-item-info">
        ...
        <div class="servico-item-meta">${s.duracao_minutos} min${s.descricao ? ' · ' + escapeHtml(s.descricao) : ''}</div>
      </div>
...
function renderTimeSlots() {
  ...
  const duracao = state.servicoSelecionado.duracao_minutos || 30;
...
async function confirmarAgendamento(e) {
  ...
  const fim = new Date(inicio.getTime() + state.servicoSelecionado.duracao_minutos * 60000);
