...
function setAdminTab(tab) {
  ...
  document.getElementById('adminTabWebhooks')?.classList.toggle('active', tab === 'webhooks');
  ...
  const statsView = document.getElementById('adminViewStats');
  if (statsView) statsView.style.display = tab === 'stats' ? 'block' : 'none';
  const webhooksView = document.getElementById('adminViewWebhooks');
  if (webhooksView) webhooksView.style.display = tab === 'webhooks' ? 'block' : 'none';

  if (tab === 'plans') loadPlans();
  if (tab === 'coupons') loadCoupons();
  if (tab === 'stats') renderAdminStats();
  if (tab === 'webhooks') renderAdminWebhooks();
}
...
// ============================================
// ABA: MÉTRICAS DO NEGÓCIO
// ============================================
async function renderAdminStats() {
  const container = document.getElementById('statsContent');
  container.innerHTML = `<div style="text-align:center;padding:40px;color:var(--text-soft)">Carregando métricas...</div>`;

  const { data: m, error } = await sb.rpc('admin_dashboard_metrics');

  if (error || !m || m.error) {
    container.innerHTML = `<div style="color:var(--red);padding:30px;text-align:center">Erro: ${error?.message || m?.error}</div>`;
    return;
  }
  ...
  container.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(220px, 1fr));gap:14px;margin-bottom:20px">
      <div class="stat-card gold">
        ...
        <div class="stat-num" style="font-size:32px">${formatHuge(m.mrr)}</div>
        <div class="stat-foot">${m.active_subscriptions} assinatura(s) ativa(s)</div>
      </div>
      <div class="stat-card">
        <div class="stat-label">ARR Anualizado</div>
        <div class="stat-num" style="font-size:28px">${formatHuge(m.arr)}</div>
        <div class="stat-foot">MRR × 12</div>
      </div>
      <div class="stat-card">
        <div class="stat-label">Ticket médio</div>
        <div class="stat-num" style="font-size:24px">${fmtBRL(m.avg_ticket)}</div>
      </div>
      <div class="stat-card" style="border-color:var(--green)">
        <div class="stat-label">Recebido últimos 30d</div>
        <div class="stat-num" style="font-size:24px;color:var(--green)">${formatHuge(m.paid_30d)}</div>
      </div>
    </div>

    <!-- Distribuição de subscriptions -->
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:20px">
      <div class="card">
        <div class="block-h"><h3>Distribuição de status</h3></div>
        <div style="display:grid;gap:10px">
          <div style="display:flex;justify-content:space-between;align-items:center;padding:10px;background:rgba(111,207,151,0.08);border-radius:8px">
            <span>✅ Ativas</span>
            <strong style="color:var(--green);font-family:'Playfair Display';font-size:18px">${m.active_subscriptions}</strong>
          </div>
          <div style="display:flex;justify-content:space-between;align-items:center;padding:10px;background:rgba(212,168,87,0.08);border-radius:8px">
            <span>🎁 Em trial</span>
            <strong style="color:var(--gold);font-family:'Playfair Display';font-size:18px">${m.trial_subscriptions}</strong>
          </div>
          <div style="display:flex;justify-content:space-between;align-items:center;padding:10px;background:rgba(255,183,62,0.08);border-radius:8px">
            <span>⚠️ Pagamento atrasado</span>
            <strong style="color:#ffb73e;font-family:'Playfair Display';font-size:18px">${m.past_due_subscriptions}</strong>
          </div>
          <div style="display:flex;justify-content:space-between;align-items:center;padding:10px;background:rgba(224,116,116,0.08);border-radius:8px">
            <span>❌ Canceladas</span>
            <strong style="color:var(--red);font-family:'Playfair Display';font-size:18px">${m.cancelled_subscriptions}</strong>
          </div>
        </div>
      </div>

      <div class="card">
        <div class="block-h"><h3>Tipo de cliente</h3></div>
        <div style="display:grid;gap:10px">
          <div style="display:flex;justify-content:space-between;align-items:center;padding:10px;background:var(--bg-soft);border-radius:8px">
            <span>🏪 Barbearia</span>
            <strong style="color:var(--gold);font-family:'Playfair Display';font-size:18px">${m.barbearia_active}</strong>
          </div>
          <div style="display:flex;justify-content:space-between;align-items:center;padding:10px;background:var(--bg-soft);border-radius:8px">
            <span>👤 Autônomo</span>
            <strong style="font-family:'Playfair Display';font-size:18px">${m.autonomo_active}</strong>
          </div>
          <div style="display:flex;justify-content:space-between;align-items:center;padding:10px;background:var(--bg-soft);border-radius:8px;border-top:1px solid var(--line);margin-top:6px">
            <span>Total de barbearias</span>
            <strong style="font-family:'Playfair Display';font-size:18px">${m.total_barbershops}</strong>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- Crescimento -->
    <div class="card">
      <div class="block-h"><h3>📈 Crescimento</h3></div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
        <div style="text-align:center;padding:20px;background:var(--bg-soft);border-radius:10px">
          <div style="font-family:'Playfair Display';font-size:36px;font-weight:700;color:var(--gold)">${m.new_users_30d}</div>
          <div style="font-size:12px;color:var(--text-soft);margin-top:4px">novos usuários nos últimos 30 dias</div>
        </div>
        <div style="text-align:center;padding:20px;background:var(--bg-soft);border-radius:10px">
          <div style="font-family:'Playfair Display';font-size:36px;font-weight:700">${m.total_users}</div>
          <div style="font-size:12px;color:var(--text-soft);margin-top:4px">usuários totais (não-admin)</div>
        </div>
      </div>
    </div>
  `;
...
// ============================================
// ABA: WEBHOOKS ASAAS (logs)
// ============================================
async function renderAdminWebhooks() {
  const container = document.getElementById('webhooksContent');
  container.innerHTML = `<div style="text-align:center;padding:40px;color:var(--text-soft)">Carregando logs...</div>`;

  const { data, error } = await sb.rpc('admin_list_webhook_logs', { p_limit: 50 });

  if (error) {
    container.innerHTML = `<div style="color:var(--red);padding:30px;text-align:center">Erro: ${error.message}</div>`;
    return;
  }
  ...
  if (!data || data.length === 0) {
    container.innerHTML = `
      <div class="card" style="text-align:center;padding:40px;color:var(--text-soft)">
        ...
        <h3 style="font-family:'Playfair Display';margin-bottom:8px">Nenhum webhook recebido ainda</h3>
        <p style="font-size:13px">Quando o Asaas enviar notificações (pagamentos, cancelamentos, etc), elas aparecerão aqui.</p>
        <div style="margin-top:20px;padding:14px;background:var(--bg-soft);border-radius:8px;font-size:11px;color:var(--text-dim);text-align:left">
          <strong>💡 Como testar:</strong><br>
          1. Acesse o painel Asaas SANDBOX<br>
          2. Vá em Cobranças<br>
          3. Marque um pagamento como recebido manualmente<br>
          4. Volte aqui e clique "Atualizar"
        </div>
      </div>
    `;
    return;
  ...
  const fmtBRL = (n) => n ? 'R$ ' + Number(n).toFixed(2).replace('.', ',') : '—';
  const fmtDate = (dt) => new Date(dt).toLocaleString('pt-BR');

  // Cores por tipo de evento
  const eventColor = (type) => {
    if (type.includes('PAYMENT_RECEIVED') || type.includes('PAYMENT_CONFIRMED')) return 'var(--green)';
    if (type.includes('OVERDUE')) return '#ffb73e';
    if (type.includes('DELETED') || type.includes('REFUND')) return 'var(--red)';
    return 'var(--text-soft)';
  };

  container.innerHTML = `
    <div style="font-size:11px;color:var(--text-dim);margin-bottom:14px">${data.length} evento(s) recente(s)</div>
        ...
        <div class="card" style="padding:14px">
          <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:10px">
            <div style="flex:1;min-width:200px">
              <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;flex-wrap:wrap">
                <span style="font-family:'JetBrains Mono', monospace;font-size:11px;color:${eventColor(log.event_type)};font-weight:600">${log.event_type}</span>
                ${log.processed ? '<span style="font-size:10px;color:var(--green)">✅ processado</span>' : '<span style="font-size:10px;color:#ffb73e">⏳ pendente</span>'}
              </div>
              <div style="font-size:11px;color:var(--text-dim);font-family:'JetBrains Mono', monospace">
                ${log.payment_id ? `payment: ${log.payment_id}` : ''}
                ${log.subscription_id ? ` · sub: ${log.subscription_id}` : ''}
                ${log.customer_id ? ` · cus: ${log.customer_id}` : ''}
              </div>
            </div>
            <div style="text-align:right">
              ${log.amount ? `<div style="font-family:'Playfair Display';font-size:18px;font-weight:700;color:var(--gold)">${fmtBRL(log.amount)}</div>` : ''}
              <div style="font-size:10px;color:var(--text-dim);margin-top:2px">${fmtDate(log.received_at)}</div>
            </div>
          </div>
          ${log.processing_error ? `<div style="margin-top:8px;padding:8px;background:rgba(224,116,116,0.08);border-radius:6px;font-size:11px;color:var(--red)">⚠️ ${log.processing_error}</div>` : ''}
        </div>
      `).join('')}
    </div>
  `;
...
window.renderAdminWebhooks = renderAdminWebhooks;
